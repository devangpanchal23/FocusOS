...
export function enqueue(row: NewEventRow): number {
  ...
  const stmt = database.prepare(`
    INSERT INTO events (occurredAt, appName, windowTitle, isIdle, durationSeconds, syncStatus)
    VALUES (@occurredAt, @appName, @windowTitle, @isIdle, @durationSeconds, 'QUEUED')
  `);
