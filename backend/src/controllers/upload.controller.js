...
export class UploadController {
     ...
     */
    static async uploadScreenshots(req, res) {
        try {
     ...
     */
    static async getExtraction(req, res) {
        try {
            const { id } = req.params;
        ...
        try {
            ...
            for (const f of extraction.fields) {
                ...
                if (f.fieldKey === 'notifications') {
                    ...
                    const finalActiveMinutes = edit?.activeMinutes !== undefined
                        ? edit.activeMinutes
                        : parseDurationToMinutes(finalDurationStr);
                    const finalBgMinutes = edit?.backgroundMinutes !== undefined
                    ...
                    const finalShortsMinutes = edit?.shortsMinutes !== undefined
                        ? edit.shortsMinutes
     ...
     * Reject extraction
     */
    static async rejectExtraction(req, res) {
        try {
            const { id } = req.params;
            const extraction = await prisma.extraction.findUnique({
                where: { id },
