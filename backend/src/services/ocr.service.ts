...
export interface ExtractedApp {
  name: string;
  duration: string; // e.g. "6h 08m"
  activeMinutes?: number;
  backgroundDuration?: string; // e.g. "1h 31m"
...
export interface ScreenshotAnalyzer {
  analyze(imagePath: string, originalFilename: string): Promise<StructuredExtractionResult>;
}

/**
 * High-accuracy pattern analyzer for Digital Wellbeing, Windows battery, & Shorts trackers
 */
export class BuiltinScreenshotAnalyzer implements ScreenshotAnalyzer {
  async analyze(imagePath: string, originalFilename: string): Promise<StructuredExtractionResult> {
    ...
    let screenshotClass: StructuredExtractionResult['screenshotClass'] = 'UNKNOWN';
    let confidence = 0.95;
