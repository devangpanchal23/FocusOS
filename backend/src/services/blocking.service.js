...
export class BlockingService {
    ...
    static async getRules(userId) {
        ...
        return rules.map((r) => {
            ...
            if (latestOverride) {
                const overrideEnd = new Date(new Date(latestOverride.createdAt).getTime() + latestOverride.overrideDurationMinutes * 60 * 1000);
                if (overrideEnd > now) {
                    isOverridden = true;
     ...
     */
    static async createOverride(userId, data) {
     ...
     */
    static async getOverrides(userId) {
