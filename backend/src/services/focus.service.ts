...
export class FocusService {
   ...
   */
  static async updateProfile(userId: string, profileId: string, data: Partial<{
    name: string;
    durationMinutes: number;
    breakMinutes: number;
    allowedApps: string[];
    ...
    if (data.allowedApps !== undefined) updateData.allowedApps = JSON.stringify(data.allowedApps);
    if (data.blockedApps !== undefined) updateData.blockedApps = JSON.stringify(data.blockedApps);
   ...
   */
  static async startSession(userId: string, data: {
    profileId?: string;
    taskName?: string;
    durationMinutes?: number;
    breakMinutes?: number;
   ...
   */
  static async recordDistraction(userId: string, sessionId: string) {
   ...
   */
  static async completeSession(userId: string, sessionId: string, data?: {
    ...
    const gamificationResult = await GamificationService.awardXP(
      userId,
      totalXP,
      `Completed ${completedMins}m session (${taskNameSnippet(session.taskName)})`
    );
   ...
   */
  static async abortSession(userId: string, sessionId: string, data?: {
    completedMinutes?: number;
    notes?: string;
  }) {
    const session = await prisma.focusSession.findFirst({
   ...
   */
  static async getSessions(userId: string, limit: number = 20) {
...
function taskNameSnippet(str: string) {
