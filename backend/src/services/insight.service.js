...
export class InsightService {
    ...
    static async generateInsights(userId) {
        ...
        if (pastMetrics.length >= 3) {
            const avgMinutes = Math.round(pastMetrics.reduce((acc, m) => acc + m.totalScreenTimeMinutes, 0) / pastMetrics.length);
        ...
        const socialApp = topUsage.find((u) => u.category?.name === 'Social Media' || u.category?.name === 'Short-form Content');
