...
export class GamificationService {
   ...
   */
  static async getGamification(userId: string): Promise<any> {
   ...
   */
  static async seedUserAchievements(userId: string) {
    const templates = [
      {
        code: 'FIRST_FOCUS',
        title: 'Deep Diver',
  ...
  static async checkAchievements(userId: string) {
    ...
    if (focusSessions.length >= 1) toUnlock.push('FIRST_FOCUS');
    if (totalFocusMinutes >= 600) toUnlock.push('DEEP_WORK_10H');
    if (zeroDistractionSessions.length >= 3) toUnlock.push('ZERO_DISTRACTIONS');
    if (userRules.length >= 1) toUnlock.push('AUTOMATION_PRO');

    const lowShortsDay = dailyMetrics.some((m) => m.shortFormMinutes > 0 && m.shortFormMinutes < 30);
    if (lowShortsDay) toUnlock.push('SHORTS_CURBED');
