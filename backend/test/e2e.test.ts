...
async function runE2ETests() {
  ...
  devData.devices.forEach((d: any) => console.log(`   - ${d.name} (${d.os} • ${d.deviceType})`));
  ...
  uploadData.uploads.forEach((u: any) =>
    console.log(`   - ${u.originalName} -> Class: ${u.classification} (Confidence: ${Math.round(u.confidence * 100)}%)`)
  );
  ...
  const updatedOverview = (await updatedOverviewRes.json()) as any;
